...
class TankController extends Controller
{
    ...
    public function bulkImportData(Request $request): JsonResponse
    {
        ...
        foreach ($collections as $key => $collection) {
            ...
            if ($collection['selling_price'] <= Helpers::discount_calculate($tank, $collection['selling_price'])) {
                return response()->json(['message' => translate('Discount can not be more or equal to the price in row') . ($key + 2)], 403);
            }
            $tank =  $this->tank->where('product_code', $collection['product_code'])->first();
            if ($tank) {
        ...
        foreach ($collections as $collection) {
            $product =  $this->tank->where('product_code', $collection['product_code'])->first();
            if ($product) {
    ...
    {
        $products = $this->tank->with('supplier', 'category', 'brand')->latest()->get();
    ...
    public function barcodeGenerate(Request $request)
    {
        ...
        $product = $this->tank->where('id', $request->id)->first();
    ...
    public function categoryWiseProduct(Request $request): JsonResponse
    {
        ...
        $categoryWiseProduct = $this->tank->with('supplier')->active()
    ...
    public function codeSearch(Request $request): JsonResponse
    {
        ...
        $tankByCode = $this->tank->latest()->paginate($limit, ['*'], 'page', $offset);
        $tanks = TanksResource::collection($tankByCode);
    ...
    public function productSort(Request $request)
    {
        ...
        $sortProducts = $this->tank->orderBy('selling_price', $sort)->latest()->paginate($limit, ['*'], 'page', $offset);
        $products = TanksResource::collection($sortProducts);
    ...
    {
        $sort = $request['sort'] ? $request['sort'] : 'ASC';
        $products = $this->tank->orderBy('order_count', $sort)->get();
        $products = tanksResource::collection($products);
        return response()->json($products, 200);
    }
    ...
    public function supplierWiseProduct(Request $request): JsonResponse
    {
        $limit = $request['limit'] ?? 10;
        $offset = $request['offset'] ?? 1;
        $product = $this->tank->where('supplier_id', $request->supplier_id)->latest()->paginate($limit, ['*'], 'page', $offset);
        $products = TankController::collection($product);
        $data = [
