...
class ProductNew extends Model
{
    ...
    public function getImageFullPathAttribute(): string
    {
        ...
        if (!is_null($image) && Storage::disk('public')->exists('product/' . $image)) {
            $path = asset('storage/app/product/' . $image);
        }
        return $path;
    }
    ...
    {
        return $this->belongsToMany(Shop::class)->withPivot('quantity','absolute', 'transaction_new_id');
    }

    public function tanks()
    {
        return $this->belongsToMany(Tank::class)->withPivot('quantity', 'absolute','transaction_new_id');
    }
