...
<head>
    ...
    <title>{{\App\CPU\translate('purchase')}}</title>
                    ...
                    <li class="nav-item d-sm-inline-block">
                        <div class="hs-unfold">
                            <a data-toggle="tooltip" class="js-hs-unfold-invoker btn btn-icon btn-ghost-secondary rounded-circle"
                                href="{{route('admin.pos.purchase-records')}}" target="_blank" title="{{\App\CPU\translate('purchase_records')}}">
            ...
            <div class="container-fluid">
                <div class="d-flex flex-wrap">
                    <div class="order--pos-left">
                        <div class="card">
                            <h5 class="p-3 m-0 bg-light">{{\App\CPU\translate('purchase_section')}}</h5>
                            <div class="px-3 py-4">
                                <div class="row gy-1">
                                    <div class="col-sm-6">
                            ...
                            <div class="card-body pt-2" id="items">
                                <div class="pos-item-wrap">
                                    @foreach($products as $product)
                                        @include('admin-views.pos._single_purchase',['product'=>$product])
                                    @endforeach
                                </div>
                                @if(count($products)==0)
                    ...
                    <div class="order--pos-right">
                        <div class="card billing-section-wrap">
                                ...
                                <div class="card-body pb-0">
                                    ...
                                    <div class="mb-3">
                                        <label class="input-label text-capitalize" >
                                            {{\App\CPU\translate('current_supplier')}} :
                                    ...
                                    <div class="d-flex gap-2 flex-wrap align-items-center mb-3">
                                        ...
                                        <div>
                                            <a class="w-i6 d-inline-block btn btn-danger rounded" href="{{route('admin.pos.clear-cart-ids-purchase')}}">
                                                {{ \App\CPU\translate('clear_cart')}}
                                            </a>
                                        </div>

                                        <div>
                                            <a class="w-i6 d-inline-block btn btn-success rounded" href="{{route('admin.pos.new-cart-id-purchase')}}">
                            ...
                            <div id="cart">
                                @include('admin-views.pos._purchase_cart',['cart_id'=>$cartId])
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    function payment_option_purchase(val) {
    ...
    function customer_change(val) {
        $.post({
                url: '{{route('admin.pos.remove-coupon-purchase')}}',
                data: {
                    _token: '{{csrf_token()}}',
                    user_id:val
                },
    ...
    {
        let  cart_id = val;
        let url = "{{route('admin.pos.change-cart-purchase')}}"+'/?cart_id='+val;
    ...
    function extra_discount()
    {
        ...
        if(discount)
        {
            ...
            $.post({
                url: '{{ route('admin.pos.discount-purchase') }}',
                data: {
                    _token: '{{csrf_token()}}',
                    discount:discount,
                    type:type,
    ...
    function coupon_discount()
    {
        ...
        $.ajaxSetup({
            ...
            $.post({
                url: '{{route('admin.pos.coupon-discount-purchase')}}',
                data: {
                    _token: '{{csrf_token()}}',
    ...
    function quickView(product_id) {
        $.ajax({
            url: '{{route('admin.pos.quick-view-purchase')}}',
            type: 'GET',
            data: {
    ...
    function addToCart(form_id) {
        ...
        let productQty = $('#product_qty').val();
            ...
            $.post({
                url: '{{ route('admin.pos.add-to-cart-purchase') }}',
                data: {
                    _token: '{{csrf_token()}}',
                    id:productId,
                    quantity:productQty,
    ...
    function removeFromCart(key) {
        $.post('{{ route('admin.pos.remove-from-cart-purchase') }}', {_token: '{{ csrf_token() }}', key: key}, function (data) {
    ...
    function emptyCart() {
        Swal.fire({
            ...
            if (result.value) {
                $.post('{{ route('admin.pos.emptyCartPurchase') }}', {_token: '{{ csrf_token() }}'}, function (data) {
                    $('#cart').empty().html(data.view);
                    $('#search').focus();
                    if(data.user_type === 'sc')
                    {
    ...
    function updateCart() {
        $.post('<?php echo e(route('admin.pos.cart_items_purchase')); ?>', {_token: '<?php echo e(csrf_token()); ?>'}, function (data) {
    ...
    function updateQuantity(id,qty) {
        setTimeout(function(){
            ...
            url: '{{ route('admin.pos.updateQuantityPurchase') }}',
            data: {
                _token: '{{csrf_token()}}',
                key: id,
                quantity: qty,
    ...
    jQuery(".search-bar-input").on('keyup',function () {
        ...
        if (name.length >0) {
            ...
            $.get({
                url: '{{route('admin.pos.search-products-purchase')}}',
                dataType: 'json',
                data: {
                    name: name
                },
    ...
    jQuery(".search-bar-input").on('keyup',delay(function () {
        ...
        if (name.length > 0 || isNaN(name)) {
            $.get({
                url: '{{route('admin.pos.search-by-add-purchase')}}',
                dataType: 'json',
                data: {
                    name: name
                },
