    ...
    <a data-id="{{ $product->id }}" class="pos-product-item card single-cart-data">
        ...
        <div class="pos-product-item_content">
            ...
            <div class="pos-product-item_price">
                {{ ($product['purchase_price']- \App\CPU\Helpers::discount_calculate($product, $product['selling_price'])) . ' ' . \App\CPU\Helpers::currency_symbol() }}

                @if($product->discount > 0)
                    <span class="fz-10 text-muted text-decoration">
                        {{ $product['purchase_price'] . ' ' . \App\CPU\Helpers::currency_symbol() }}
                    </span><br>
                @endif
            </div>
        </div>
