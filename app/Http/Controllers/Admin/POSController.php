...
class POSController extends Controller
{
    ...
    public function purchase(Request $request): Factory|View|Application
    {
        $category = $request->query('category_id', 0);
        $keyword = $request->query('search', false);
        $key = explode(' ', $keyword);
        ...
        // $products = $this->product->where('quantity', '>', 0)->active() //removed BY PSP
        ...
        return view('admin-views.pos.purchase', compact('categories', 'products', 'cartId', 'category', 'userId'));
    }
    ...
    public function clearCartIdsPurchase(): RedirectResponse
    {
        session()->forget('cart_name');
        session()->forget(session('current_user'));
        session()->forget('current_user');

        return redirect()->route('admin.pos.purchase');
    } //added by PSP
    ...
    public function quickViewPurchase(Request $request): JsonResponse
    {
        $product = $this->product->findOrFail($request->product_id);

        return response()->json([
            'success' => 1,
            'view' => view('admin-views.pos._quick-view-data-purchase', compact('product'))->render(),
        ]);
    } //added BY PSP 
    ...
    public function addToCartPurchase(Request $request): JsonResponse
    {
        $cartId = session('current_user');
        $userId = 0;
        $userType = 'wc';
                ...
                if (is_array($cartItem) && $cartItem['id'] == $request['id']) {
                    ...
                    if ($qty == 0) {
                        return response()->json([
                            ...
                            'user_type' => $userType,
                            'user_id' => $userId,
                            'view' => view('admin-views.pos._purchase_cart', compact('cartId'))->render()
                        ]);
                    }
                }
            }
    ...
    public function cartItemsPurchase(): Factory|View|Application
    {
        return view('admin-views.pos._purchase_cart');
    } //added BY PSP
    ...
    public function emptyCart(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'user_type' => $userType,
            'user_id' => $userId,
            'view' => view('admin-views.
            
            
            
            ._cart', compact('cartId'))->render()
        ], 200);
    }

    public function emptyCartPurchase(Request $request): JsonResponse
    {
        $cartId = session('current_user');
        $userId = 0;
        $userType = 'wc';
        ...
        return response()->json([
            ...
            ._purchase_cart', compact('cartId'))->render()
        ], 200);
    } //added BY PSP
    ...
    public function removeFromCartPurchase(Request $request): JsonResponse
    {
        $cartId = session('current_user');
        $userId = 0;
        $userType = 'wc';
    ...
    public function updateDiscountPurchase(Request $request): JsonResponse
    {
        $cartId = session('current_user');
        $userId = 0;
        $userType = 'wc';
    ...
    public function couponDiscountPurchase(Request $request): JsonResponse
    {
        $cartId = session('current_user');
        $userId = 0;
        $userType = 'wc';
    ...
    public function placeOrderPurchase(Request $request): RedirectResponse
    {
        $cartId = session('current_user');
        $userId = 0;
        $userType = 'wc';
            ...
            if (is_array($c)) {
                ...
                if ($product) {
                    ...
                    $orD = [
                        ...
                        'tax_amount' => Helpers::tax_calculate($product, $product->purchase_price),
                        'discount_on_product' => Helpers::discount_calculate($product, $product->purchase_price),
                        'discount_type' => 'discount_on_product',
                        'created_at' => now(),
                        'updated_at' => now()
                    ];
    ...
    //                     $receivableTransaction->debit = 0;
    //                     $receivableTransaction->credit = 1;
    //                     $receivableTransaction->balance = $receivableAccount->balance + $request->remaining_balance; //modified BY PSP 
    //                     $receivableTransaction->date = date("Y/m/d");
    //                     $receivableTransaction->customer_id = $customer->id;
    //                     $receivableTransaction->order_id = $order_id;
    //                     $receivableTransaction->save();

    //                     $receivableAccount->total_in = $receivableAccount->total_in + $request->remaining_balance; //modified BY PSP 
    //                     $receivableAccount->balance = $receivableAccount->balance + $request->remaining_balance; //modified BY PSP 
    //                     $receivableAccount->save();
    //                 } else {
    ...
    //                     $receivableTransaction->debit = 0;
    //                     $receivableTransaction->credit = 1;
    //                     $receivableTransaction->balance = $receivableAccount->balance - $grandTotal; //modified BY PSP 
    //                     $receivableTransaction->date = date("Y/m/d");
    //                     $receivableTransaction->customer_id = $customer->id;
    //                     $receivableTransaction->order_id = $order_id;
    //                     $receivableTransaction->save();

    //                     $receivableAccount->total_in = $receivableAccount->total_in - $grandTotal; //modified BY PSP 
    //                     $receivableAccount->balance = $receivableAccount->balance - $grandTotal; //modified BY PSP 
    //                     $receivableAccount->save();
    //                 }
    //             }
    ...
    //             $transection->tran_type = 'Income';
    //             $transection->account_id = $request->type;
    //             $transection->amount = $totalPrice - $totalTaxAmount + $extDiscount + $couponDiscount; //modified BY PSP 
    //             $transection->description = 'POS order';
    //             $transection->debit = 0;
    //             $transection->credit = 1;
    //             $transection->balance = $account->balance - ($totalPrice + $totalTaxAmount - $extDiscount - $couponDiscount);
    ...
    public function searchProductPurchase(Request $request): JsonResponse
    {
        $request->validate([
            'name' => 'required',
        ], [
        ...
        return response()->json([
            'result' => view('admin-views.pos._search-result-purchase', compact('products'))->render(),
            'count' => $countP
        ]);
    } //added BY PSP
    ...
    public function searchByAddProductPurchase(Request $request)
    {
        $request->validate([
            'name' => 'required',
        ], [
    ...
    public function removeCouponPurchase(Request $request): JsonResponse
    {
        $cartId = ($request->user_id != 0 ? 'sc-' . $request->user_id : 'wc-' . rand(10, 1000));
        if (!in_array($cartId, session('cart_name') ?? [])) {
            session()->push('cart_name', $cartId);
    ...
    public function changeCartPurchase(Request $request): RedirectResponse
    {
    ...
    public function newCartIdPurchase(Request $request): RedirectResponse
    {
        $cartId = 'wc-' . rand(10, 1000);
        session()->put('current_user', $cartId);
        if (!in_array($cartId, session('cart_name') ?? [])) {
    ...
    public function getCartIdsPurchase(Request $request): JsonResponse
    {
        $cartId = session('current_user');
        $userId = 0;
        $userType = 'wc';
