...
class TransferRecordController extends Controller
{
    ...
    public function createTransferRecord(Request $request)
    {
        ...
        foreach ($products as $product_new) {
            $transferRecord->productNews()->attach(
                $product_new['product_new_id'],
                ['quantity' => $product_new['quantity'], 'isExtra' => 0]
            );
        }
        ...
        foreach ($extraProducts as $product_new) {
            $transferRecord->productNews()->attach(
                $product_new['product_new_id'],
                ['quantity' => $product_new['quantity'], 'isExtra' => 1]
            );
        }
        ...
        foreach ($products as $product_new) {
            if ($from == 'shop') {
                $senderCurrentQuantity = $fromShop->product_news()->where(
                    'product_new_id',
                    $product_new['product_new_id']
                )->first()->pivot->absolute;
                $newQuantity = intval($senderCurrentQuantity) - intval($product_new['quantity']);
                $latestTransactionNewId = $fromShop->product_news()
                    ->where('product_new_id', $product_new['product_new_id'])
                    ->orderBy('transaction_new_id', 'desc')
                    ->first()
                    ->pivot
                    ->transaction_new_id;

// Update the pivot table using the latest transaction_new_id
                $fromShop->product_news()->wherePivot(
                    'transaction_new_id',
                    $latestTransactionNewId
                )->updateExistingPivot($product_new['product_new_id'], [
...
//                ]);
                ...
                $senderCurrentQuantity = $fromTank->productNews()->where(
                    'product_new_id',
                    $product_new['product_new_id']
    ...
    public function update(Request $request)
    {
        ...
        foreach ($actualProducts as $product_new) {
            $transferRecord->productNews()->attach(
                $product_new['product_new_id'],
                ['quantity' => $product_new['quantity'], 'isExtra' => 2]
            );
        }
        ...
        foreach ($actualProducts as $product_new) {
            if (intval($isFinal) == 0) {
                if ($toTank->product_news()->where('product_new_id', $product_new['product_new_id'])->exists()) {
                    $recieverCurrentQuantity = $toTank->productNews()->where(
                        'product_new_id',
                        $product_new['product_new_id']
                    )->first()->pivot->quantity;
...
//            $transferRecord->productNews()->attach($product_new['product_new_id'], ['quantity' => $product_new['quantity'], 'isExtra' => 2]);
            ...
            $transferRecord->productNews()->wherePivot('isExtra', 2)->updateExistingPivot(
                $product_new['product_new_id'],
                [
...
//
//// Update the pivot table using the latest transaction_new_id
//                $fromShop->product_news()->wherePivot('transaction_new_id', $latestTransactionNewId)->updateExistingPivot($product_new['product_new_id'], [
//                    'absolute' => $newQuantity,
//                ]);
////                $fromShop->product_news()->updateExistingPivot($product_new['product_new_id'], [
////                    'absolute' => $newQuantity,
