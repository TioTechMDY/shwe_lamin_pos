...
class ShopController extends Controller
{
    ...
    public function bulkImportData(Request $request): JsonResponse
    {
        ...
        foreach ($collections as $key => $collection) {
            ...
            if ($collection['selling_price'] <= Helpers::discount_calculate($shop, $collection['selling_price'])) {
                return response()->json(['message' => translate('Discount can not be more or equal to the price in row') . ($key + 2)], 403);
            }
            $shop =  $this->shop->where('product_code', $collection['product_code'])->first();
            if ($shop) {
        ...
        foreach ($collections as $collection) {
            $product =  $this->shop->where('product_code', $collection['product_code'])->first();
            if ($product) {
    ...
    {
        $products = $this->shop->with('supplier', 'category', 'brand')->latest()->get();
    ...
    public function barcodeGenerate(Request $request)
    {
        ...
        $product = $this->shop->where('id', $request->id)->first();
    ...
    public function categoryWiseProduct(Request $request): JsonResponse
    {
        ...
        $categoryWiseProduct = $this->shop->with('supplier')->active()
    ...
    public function codeSearch(Request $request): JsonResponse
    {
        ...
        $shopByCode = $this->shop->latest()->paginate($limit, ['*'], 'page', $offset);
        $shops = ShopsResource::collection($shopByCode);
    ...
    public function productSort(Request $request)
    {
        ...
        $sortProducts = $this->shop->orderBy('selling_price', $sort)->latest()->paginate($limit, ['*'], 'page', $offset);
        $products = ShopsResource::collection($sortProducts);
    ...
    {
        $sort = $request['sort'] ? $request['sort'] : 'ASC';
        $products = $this->shop->orderBy('order_count', $sort)->get();
        $products = ShopsResource::collection($products);
        return response()->json($products, 200);
    }
    ...
    public function supplierWiseProduct(Request $request): JsonResponse
    {
        $limit = $request['limit'] ?? 10;
        $offset = $request['offset'] ?? 1;
        $product = $this->shop->where('supplier_id', $request->supplier_id)->latest()->paginate($limit, ['*'], 'page', $offset);
        $products = ShopController::collection($product);
        $data = [
